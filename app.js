...
app.get("/faculty", (req, res) => {
  ...
  jwt.verify(token, JWT_SECRET, (err, decoded) => {
    ...
    con.query(query, [email], (err, result) => {
      if (err) {
        ...
        if (result[0].role === "Faculty") {
            ...
            return res.render("error",{message:"Unauthorized login : Not a faculty" });
          }
    })
    ...
    con.query(checkQuery, [email], (err, result) => {
      if (err) {
        return res.status(500).render("error",{ message: 'Database query failed' });
      }
      ...
      if (result.length === 0) {
        ...
        con.query(insertQuery, [email], (err, result) => {
          if (err) {
            return res.status(500).render("error",{ message: 'Failed to insert faculty' });
          }
          console.log("Faculty added");
          const recheckQuery = "SELECT department_id FROM instructor WHERE email = ?";
...
app.post("/faculty",(req,res)=>{
  ...
  const query = "UPDATE instructor SET department_id = ?, first_name = ?, last_name = ? WHERE email = ?";
...
app.get("/student", (req, res) => {
  ...
  jwt.verify(token, JWT_SECRET, (err, decoded) => {
    ...
    const checkQuery = "SELECT first_name, last_name FROM student WHERE email = ?";
    con.query(checkQuery, [email], (err, result) => {
      if (err) {
        return res.status(500).render("error",{ message: 'Database query failed' });
      }
      ...
      if (result.length === 0) {
        ...
        con.query(insertQuery, [email], (err, result) => {
          if (err) {
            return res.status(500).render("error",{ message: 'Failed to insert student' });
          }
          console.log("Student added");
          const recheckQuery = "SELECT first_name, last_name FROM student WHERE email = ?";
          con.query(recheckQuery, [email], (err, result) => {
            if (err) {
              return res.status(500).render("error",{ message: 'Database query failed' });
            }

            if (result.length === 0 || result[0].first_name === null || result[0].last_name === null) {
...
app.post("/student",(req,res)=>{
  ...
  const query = "UPDATE student SET first_name = ?, last_name = ? WHERE email = ?";
...
app.get("/examSchedule", (req, res) => {
  ...
  jwt.verify(token, JWT_SECRET, (err, decoded) => {
    ...
    const examSchedule = [
      { course_name: "Introduction to Computer Science", exam_date: "2024-10-01", exam_time: "09:00 AM" },
      { course_name: "Data Structures", exam_date: "2024-10-05", exam_time: "01:00 PM" },
      { course_name: "Database Systems", exam_date: "2024-10-10", exam_time: "11:00 AM" },
      { course_name: "Web Development", exam_date: "2024-10-15", exam_time: "02:00 PM" }
    ];
...
app.get('/facultyMarks', (req, res) => {
  ...
  jwt.verify(token, JWT_SECRET, (err, decoded) => {
    ...
    const getCoursesQuery = 'SELECT course_id FROM instructor_courses WHERE instructor_email = ?';
...
app.post('/updateMarks', (req, res) => {
  ...
  con.query(updateMarksQuery, [student_email, course_id, marks_obtained, marks_obtained], (err, result) => {
    if (err) {
...
app.post("/login", (req, res) => {
  ...
  con.query(query, [username], (err, rows) => {
    if (err) return res.status(500).render("error",{ message: 'Internal Server Error' });
    if (rows.length === 0) return res.status(401).render("error",{ message: 'Invalid username or password' });
...
app.post("/enrollCoursesFaculty", (req, res) => {
  ...
  jwt.verify(token, JWT_SECRET, (err, decoded) => {
    ...
    const enrollmentQuery = "INSERT INTO instructor_courses (instructor_email, course_id) VALUES (?, ?)";
    con.query(enrollmentQuery, [email, courseId], (err, result) => {
      if (err) {
        return res.status(500).render("error",{ message: 'Failed to assign course' });
      }
...
app.post('/enrollCourses', (req, res) => {
  ...
  jwt.verify(token, JWT_SECRET, (err, decoded) => {
    ...
    const insertQuery = 'INSERT INTO student_courses (email, course_id) VALUES (?, ?)';
...
con.query(query, [email], (err, enrolledCourses) => {
  ...
  res.render('enrolledCourses', { email: email, enrolledCourses: enrolledCourses });
});
  });
});
...
app.post('/updateCourse/:course_id', (req, res) => {
  ...
  const updateQuery = 'UPDATE student_courses SET course_id = ? WHERE course_id = ?';
...
app.get('/deleteCourses', (req, res) => {
  ...
  jwt.verify(token, JWT_SECRET, (err, decoded) => {
    ...
    const query = 'DELETE FROM student_courses WHERE course_id = ? AND email = ?';
...
app.post("/updateAssignedCourse", (req, res) => {
  ...
  // Update the course
  const updateQuery = "UPDATE instructor_courses SET course_id = ? WHERE course_id = ?";
  con.query(updateQuery, [course_id, old_course_id], (err) => {
    if (err) {
      return res.status(500).render("error",{ message: 'Failed to update course' });
    }
  ...
  jwt.verify(token, JWT_SECRET, (err, decoded) => {
    ...
    con.query(getFacultyIdQuery, [email], (err, result) => {
      ...
      con.query(deleteQuery, [course_id, facultyEmail], (err, result) => {
        if (err) {
          return res.status(500).render("error",{ message: 'Failed to delete course' });
        }
...
app.post('/updateCourse', (req, res) => {
  ...
  const query = 'UPDATE Course SET course_name = ?, department_id = ? WHERE course_id = ?';
...
app.post('/updateUser', (req, res) => {
  const { email, first_name, last_name, role } = req.body;
    ...
    let updateQuery;
    if (role === 'Student') {
      updateQuery = 'UPDATE Student SET first_name = ?, last_name = ? WHERE email = ?';
    } else if (role === 'Faculty') {
      updateQuery = 'UPDATE Instructor SET first_name = ?, last_name = ? WHERE email = ?';
    }
    ...
    if (updateQuery) {
      con.query(updateQuery, [first_name, last_name, email], (err) => {
        if (err) {
          return res.status(500).send('Error updating user details: ' + err.message);
        }
  ...
  con.query(userRoleQuery, [userId], (err, results) => {
    ...
    if (deleteAssociatedRecordsQuery) {
      con.query(deleteAssociatedRecordsQuery, [userId], (err) => {
        if (err) {
          return res.status(500).send('Error deleting associated records: ' + err);
        }
...
app.get('/viewReports', (req, res) => {
  ...
  con.query(`${studentQuery}; ${facultyQuery}; ${courseQuery}; ${departmentQuery}`, (err, results) => {
      if (err) {
